...
fun <- function(a) {
  ...
  if (!is.numeric(a)) {
    warning("Error: This not a numeric vector.")
  }    
  ...
  #return both a and the calculated values that are sorted
  return(list(stats, stats_sorted))
...
#finding total size and mean
ny.counties %>%
  as_tibble() %>%
  select(-geometry) %>%
  mutate(totalcountyArea = (AWATER10 + ALAND10)) %>%
  summarise(avg_size = mean(totalcountyArea, na.rm=TRUE))
...
station_location %>% 
dplyr::slice_max(order_by = avg_drainage, n = 1, with_ties = FALSE) %>% dplyr::select(STATEFP10) %>% head() 
```
...
```{r}
check1 <- sf::st_intersection(d.stations, del.counties)
check2 <- sf::st_intersection(del.counties, d.stations)
```
They technically give the same response, but the differences is what type of data will be retained. In the first scenario, station data will be preserved and it will find what stations are in Delaware counties. In the second scenario, it preserves del.counties data and it is useful to see if there are stations in those counties. I think this would be different if we had two objects with very different shapes or geometries like if one was lines and the other was polygons. Depending on the intersection order, you would retain the shape of the first object.


2. What did you find challenging in this lab? What was new?
I think it was challenging that this lab didn't have clear meta data associated with it. Overall it was a good structure and I think it was challenging but I was capable of doing it mostly correct (I hope).

3. What types of activities would you like to see in labs this semester?
I would like to do activities like this that use real data and would be realistic problems we would face. I think it would also be cool to see some code that you have done that was tricky and how you found your solution.
...
group_by(STATEFP10) %>%
  mutate(stateLandArea = sum(ALAND10)) %>%
  mutate(stateWaterArea = sum(AWATER10)) %>%
  mutate(totalStateArea = sum(stateLandArea + stateWaterArea))

#calculating county area as a percentage of total area
#start by calculating county area
new.d.counties <- new.d.counties %>%
group_by(COUNTYFP10) %>%
  mutate(countyLandArea = sum(ALAND10))%>%
  mutate(countyWaterArea = sum(AWATER10)) %>%
  mutate(totalCountyArea = sum(countyLandArea + countyWaterArea))

# calculate the percentage of land
new.d.counties <- new.d.counties %>%
  mutate(countyLandPercent = (countyLandArea / totalStateArea))
```
...
new.d.counties <- new.d.counties %>%
group_by(COUNTYFP10) %>%
  mutate(countyWaterPercent = (countyWaterArea /totalCountyArea))
...
```{r}
#create column with the count of counties per state
new.d.counties  %>%

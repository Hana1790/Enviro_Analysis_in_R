...
clean_cicyano <- function(r) {
  ...
  rcl_matrix <- matrix(c(0,0,NA,
                         249,256,NA),ncol = 3, byrow = TRUE)
  
  #cleaned  up raster, setting invalid data to NA
  cleaned_raster <- terra::classify(r, rcl = rcl_matrix, others=NULL)
...
-The others function here decides that something not identified in the reclassification matrix will be assigned a NA value. NOOTTTT FINISHED!!!!!!!!!!!!
...
#mask the raster
mask_rast <- terra::mask(crop_rast, erie_sand) 
plot(mask_rast)
```

...
oh_streams <- read_sf("../week5_inclass/ohio/ohio_rivers.gpkg")
```
...
```{r}
oh_streams %>% sf::st_crs() #WGS 84/ Pseudo-mercator
```
...
```{r}
oh_counties <- read_sf("../week5_inclass/ohio/oh_counties.gpkg")
oh_counties %>% glimpse()
```
...
oh_streams %>% sf::st_crs() #WGS 84
study.area %>% sf::st_crs() #NAD 83
```
...
```{r}
oh_parks_shp <- read_sf("../week5_inclass/ohio/ohio_parks.shp")
oh_parks <- read_sf("../week5_inclass/ohio/oh_parks.gpkg")
```
...
```{r}
#need 303 d streams and ohio_parks_p
#check crs of 303d and parks
st_crs(streams.303d) == st_crs(oh_parks) #shp ohio parks

#create buffer from 303d streams (1 km)
stream_buffer <- sf::st_buffer(streams.303d, dist = 1000)

#create intersection of stream buffer and parks
ohio_streams_parks <- sf::st_intersection(oh_parks, stream_buffer)

tmap::tmap_mode()
tmap::ttm() #SWITCHES VIEW!

#plotting the parks that interesect
tm_shape(oh_counties) + tm_polygons(fill = "grey") + 
  tm_shape(streams.303d) + tm_lines(col = "purple")+
  tm_shape(ohio_streams_parks) + tm_polygons(fill = "FEATTYPE", palette = "brewer.dark2") 
...
```{r}
#original 4 counties study area

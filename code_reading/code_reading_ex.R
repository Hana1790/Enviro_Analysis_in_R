...
### Why do I need to include them first? - Have to have them written first to reference them later


### DESCRIBE HOW THING FUNCTION WORKS
collect_images <- function(im.path) { #path to NOAA files
  
  # What am I doing here?
  file_names <- fs::dir_ls(im.path) #to list directory and the file names
  ext_names <- fs::path_ext(file_names) # get file extensions - filename portion of the path
  
  # put in tibble, filter for tif only
  image.files <- data.frame(file_names, ext_names) %>%
    as_tibble() %>%
    mutate(fn_char = as.character(file_names)) %>% #turning file names into character string in new column
    mutate(file_names = fs_path(file_names %>% as.character)) %>% #lost fs type when put into df
    filter(str_detect(ext_names, "tif")) %>% #look for file names with "tif" 
    mutate(fn = fs::path_file(file_names)) %>% #make own column for selected files
    mutate(
      year = stringr::str_sub(fn, 4, 7), #extracting year from character string
      start_month = stringr::str_sub(fn, 9, 10), #extracting month
      start_day = stringr::str_sub(fn, 11, 12), #extracting day
      end_month = stringr::str_sub(fn, 14, 15), #find the end month
      end_day = stringr::str_sub(fn, 16, 17) # find end day
    ) %>%
    mutate(
...
clip_raster_to_bounding <- function(in.raster.path, boundingPolygon.path) {
  ...
  ras <- terra::rast(in.raster.path) #rasterize the raster path
  
  ras.p4 <- terra::crs(ras) #checking crs 
  
  # What's a bounding polygon and how can you know what that poly looks like?
  bb <- sf::read_sf(boundingPolygon.path) #read in our bounding box
  
  bb.projected <- sf::st_transform(bb, ras.p4) %>% vect(.) #reproject bounding box to raster and create spatial vector
  
  ras.subset <- terra::crop(ras, bb.projected) #crop the raster to the projected bounding box
}


### DESCRIBE HOW THIS FUNCTION WORKS
calc_area_by_thresholds <- function(in.raster, #start with raster, bb, and use a window of time
                                    boundingPolygon.path,
                                    start_date,
                                    end_date) {
  # read it as a simple feature
  ...
  # raster properties
  raster.res <- terra::res(in.raster) #finding out what the resolution of in.raster is
  pixel.area.m2 <- raster.res[1] * raster.res[2] #multiplies resolution of x by y
  
  
  # What does terra::crop do?
  ras.subset <- terra::crop(in.raster, bb.projected) #cut out where bb is projected
...
## path to NOAA tifs
im.path <- fs::path("./erie_cicyano/")

## bounding box path
bb.path <- fs::path("./shapes/bb_erie_westernext.shp")

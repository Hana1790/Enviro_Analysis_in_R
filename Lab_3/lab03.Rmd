...
```{r}
#creating a subset of the eastern/midwest states around Colorado (CO, UT, NM, KS, WY) and looking at household income
df.vars %>% dplyr::filter(concept == "MEDIAN HOUSEHOLD INCOME IN THE PAST 12 MONTHS") %>% 
  dplyr::filter(str_detect(label, "Female")) %>% 
  dplyr::filter(str_detect(label, "22 to 24"))

#picking the year and getting it into the county level from 2023
female.age <- get_acs(geography = "county", 
                      table = "B01001",
                      output = "wide", 
                      geometry = T, 
                      year = 2023) #variable I want is 034, but I'll get the whole table

glimpse(female.age)

# limiting this down to just those eastern midwest states
colorado_states <- female.age %>% 
  mutate(., STATEID = stringr::str_sub(GEOID, 1, 2)) %>% 
  dplyr::filter(STATEID %in% c("08","49", "35", "20", "56")) #changed to %in%


# map it to verify we got all the states!
tmap::tm_shape(colorado_states) + tm_polygons()
...
sf::st_crs(colorado_states) 

# then reproject to north american equidistant conic
colorado_states.projected <- colorado_states %>% sf::st_transform(., "ESRI:102010")

# plot it again to make sure nothing broke
tmap::tm_shape(colorado_states.projected) + tm_polygons()

# make the neighborhood!!
nb_colorado <- spdep::poly2nb(colorado_states.projected, queen = TRUE) #Construct neighbors list from polygon list (poly2nb). Chose queen to make it more permeable

...
```{r}
counties <- sf::read_sf("../Lab_2/CBW/County_Boundaries.shp") %>% sf::st_make_valid()

dams <- sf::read_sf("../Lab_2/CBW/Dam_or_Other_Blockage_Removed_2012_2017.shp") %>% sf::st_make_valid()

streams <- sf::read_sf("../Lab_2/CBW/Streams_Opened_by_Dam_Removal_2012_2017.shp") %>% sf::st_make_valid()
...
```{r}
#we only see one county for each of these dams
st_intersects(dams, pa.counties)
```

```{r}
# order matters
dams %>% st_intersects(x = ., y = pa.counties) #same as the first way
dams %>% st_intersects(x = pa.counties, y = .) #for this county, what dam is inside of it
```
Matrix output with the ```sparse``` parameter

```{r}
#this way tells us the entire matrix!
...
# Within
dams %>% st_within(., pa.counties, sparse = F) #looks like an intersection here
```

More examples
```{r}
#starts with filtering counties
c.tioga <- counties %>% 
  dplyr::filter(str_detect(NAME10, "Tioga") & STATEFP10 == 42) #detecting a string
...
```{r}
streams.tioga %>% st_is_within_distance(., dams, 1) #1 is the distance we're testing

#this is an unproject Geodatic reference system
st_crs(counties) #the 1 means degrees
```

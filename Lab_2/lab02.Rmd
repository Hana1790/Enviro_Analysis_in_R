...
#probably the best this is going to get
log_scatter
```
...
cover_crop <-
  ...
  ggplot(aes(x = StateAbbreviation, y = TotalAmountCredited)) +
    geom_boxplot(aes(fill = StateAbbreviation)) #for fun
...
dams_new <- dams |>
  filter(YEAR != 0) |> #not in the year 0
...
```{r}
#Total length of streams opened by dam removals
...
new_df <- dams |> as_tibble() |> select(-geometry) |> #prevents it from being a spatial join
  left_join(streams, by=c("DamRemoval"), relationship= "many-to-many",
            suffix=c("_dams","_streams")) #renames columns that are shared to specify

#group by dam name
new_df <- new_df |> group_by(DAM_NAME_dams) |> #renamed, new variable wasn't showing up
  mutate(TotalLengthStreams= sum(LengthKM, na.rm=TRUE), .groups="drop")  #sum length of streams
 
new_df |> ggplot() +
  geom_bar(aes(x=DAM_NAME_dams, y=TotalLengthStreams, fill=STATE_dams), stat="identity")+ #change from count to total length
  scale_fill_manual(values = c("tomato2", "tan3", "dodgerblue4"), name = "State") + #color by state
  labs(x="Removed Dam Names", y="Total Stream Length (km)",
       title="Dams and the Total Length of Streams Opened by Removal in km") +
  theme_bw()+
  theme(axis.text.x = element_text(angle=90)) #rotate text
...
#quick map to check that only selected counties are shown
...
counties_streams |> as_tibble() |> select(-geometry) |>
 group_by(COUNTYFP10) |>
  ...
  slice_max(AllStreamLength, n=3) #gives the top 3 counties
```
